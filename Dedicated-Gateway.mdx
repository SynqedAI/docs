...
- You're building a production agent that needs deterministic behavior
- You want to minimize context window usage by exposing only relevant tools
...
3. Select an **Auth Config** for that server. You can pick an existing one or click **\+ Create Auth Config** to create a new one inline (see [Auth Configs](/authentication/auth-configs) for details)
...
<Frame>
  ![Step 9 Gateway Creation](/images/step-9-gateway-creation.png)
</Frame>
